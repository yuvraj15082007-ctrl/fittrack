...
export default function FitTrack() {
  ...
  const TrackerScreen = () => (
    <div style={{ padding: "0 16px", animation: "fadeIn .3s ease" }}>
      ...
      <div style={{ ...s.card, height: 180, padding: 8, position: "relative" }}>
        ...
        {trackState === "running" && <div style={{ position: "absolute", top: 14, right: 14, background: theme.danger, color: "#fff", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 700, animation: "pulse 1.5s infinite" }}>● LIVE</div>}
        {trackState === "paused" && isAutoPaused && (
          <div style={{ position: "absolute", top: 14, right: 14, background: theme.warn, color: "#000", borderRadius: 6, padding: "2px 10px", fontSize: 11, fontWeight: 700, animation: "pulse 1.5s infinite" }}>⏸ AUTO-PAUSED</div>
        )}
      </div>
      ...
      <div style={s.card}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={{ fontSize: 13, fontWeight: 600 }}>Daily Goal — {activity}</span>
          <span style={{ fontSize: 13, color: theme.accent }}>{formatDist(todayDist)} / {formatDist(goals[activity])}</span>
        </div>
        <div style={{ background: theme.surface, borderRadius: 8, height: 8 }}>
          <div style={{ background: goalPct >= 100 ? theme.warn : theme.accent, height: 8, borderRadius: 8, width: `${goalPct}%`, transition: "width .5s" }} />
        </div>
        {goalPct >= 100 && <div style={{ color: theme.warn, fontSize: 12, marginTop: 6, textAlign: "center" }}>🎯 Goal complete! Beast mode 🔥</div>}
      </div>

      {/* Controls */}
      <div style={{ display: "flex", gap: 10 }}>
        {trackState === "idle" && (
          <button style={{ ...s.btn(theme.accent), flex: 1, fontSize: 18 }} onClick={startTracking}>▶ START</button>
        )}
        {trackState === "running" && <>
          <button style={{ ...s.btn(theme.warn), flex: 1 }} onClick={pauseTracking}>⏸ PAUSE</button>
          <button style={{ ...s.btn(theme.danger), flex: 1 }} onClick={stopTracking}>■ STOP</button>
        </>}
        {trackState === "paused" && isAutoPaused && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ background: `${theme.warn}20`, border: `1px solid ${theme.warn}50`, borderRadius: 10, padding: "10px 14px", textAlign: "center", color: theme.warn, fontSize: 13, fontWeight: 600 }}>
              🚶 Move karo → Auto-resume hoga
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button style={{ ...s.btn(theme.accent), flex: 1 }} onClick={resumeTracking}>▶ RESUME</button>
              <button style={{ ...s.btn(theme.danger), flex: 1 }} onClick={stopTracking}>■ STOP</button>
            </div>
          </div>
        )}
  ...
  const HomeScreen = () => {
    ...
    const totalRuns = history.filter(r => r.activity === "Running").length;
    const totalCyc = history.filter(r => r.activity === "Cycling").length;
    const todayActs = history.filter(r => r.date === today);
    return (
      <div style={{ padding: "0 16px", animation: "fadeIn .3s ease" }}>
        {/* Greeting */}
        <div style={{ ...s.card, background: `linear-gradient(135deg, ${theme.accent}15, ${theme.accentAlt}15)`, borderColor: `${theme.accent}30` }}>
          <div style={{ fontSize: 22, fontWeight: 700 }}>Hey, {user.name}! 👋</div>
          <div style={{ color: theme.muted, fontSize: 14, marginTop: 4 }}>{today} · {user.weight} kg</div>
          {todayActs.length > 0
            ? <div style={{ color: theme.accent, fontSize: 14, marginTop: 8 }}>🔥 {todayActs.length} activity today — keep it up!</div>
            : <div style={{ color: theme.muted, fontSize: 14, marginTop: 8 }}>No activity today yet. Let's move! 💪</div>
          }
        </div>

        {/* Total stats */}
        <div style={{ ...s.card }}>
          <div style={{ fontSize: 13, color: theme.muted, letterSpacing: 1, marginBottom: 12 }}>ALL TIME STATS</div>
          <div style={{ display: "flex" }}>
            <div style={s.stat}><div style={s.statVal}>{(totalDist/1000).toFixed(1)}</div><div style={s.statLbl}>TOTAL KM</div></div>
            <div style={{ width: 1, background: theme.border }} />
            <div style={s.stat}><div style={{ ...s.statVal, color: theme.warn }}>{totalCal}</div><div style={s.statLbl}>KCAL</div></div>
            <div style={{ width: 1, background: theme.border }} />
            <div style={s.stat}><div style={s.statVal}>{totalRuns + totalCyc}</div><div style={s.statLbl}>ACTIVITIES</div></div>
          </div>
        </div>

        {/* Quick action */}
        <button style={{ ...s.btn(theme.accent), width: "100%", fontSize: 18, marginBottom: 14 }} onClick={() => setScreen("track")}>
          ⚡ START ACTIVITY
        </button>
        ...
        {todayActs.length > 0 && (
          <div style={s.card}>
            <div style={{ fontSize: 13, color: theme.muted, letterSpacing: 1, marginBottom: 12 }}>TODAY</div>
            {todayActs.map(r => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: `1px solid ${theme.border}` }}
                onClick={() => { setActiveHistItem(r); setScreen("histDetail"); }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ fontSize: 24 }}>{r.activity === "Running" ? "🏃" : "🚴"}</div>
                  <div>
                    <div style={{ fontWeight: 600 }}>{r.activity}</div>
                    <div style={{ color: theme.muted, fontSize: 13 }}>{formatTime(r.time)} · {formatDist(r.distance)}</div>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: theme.warn, fontWeight: 700 }}>{r.calories} kcal</div>
    ...
    <div style={{ padding: "0 16px", animation: "fadeIn .3s ease" }}>
      {history.length === 0
        ? <div style={{ textAlign: "center", padding: "60px 20px", color: theme.muted }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 600 }}>No activities yet</div>
            <div style={{ fontSize: 14, marginTop: 8 }}>Start tracking to see history here</div>
          </div>
        : history.map(r => (
          <div key={r.id} style={{ ...s.card, cursor: "pointer" }} onClick={() => { setActiveHistItem(r); setScreen("histDetail"); }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ fontSize: 28 }}>{r.activity === "Running" ? "🏃" : "🚴"}</div>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16 }}>{r.activity}</div>
                  <div style={{ color: theme.muted, fontSize: 13 }}>{r.date}</div>
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontWeight: 700, color: theme.accent }}>{formatDist(r.distance)}</div>
                <div style={{ color: theme.muted, fontSize: 13 }}>{formatTime(r.time)}</div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 16, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${theme.border}` }}>
              <span style={{ color: theme.muted, fontSize: 13 }}>⚡ {(r.speed * 3.6).toFixed(1)} km/h</span>
              {r.pace && <span style={{ color: theme.muted, fontSize: 13 }}>👟 {formatPace(r.pace)}</span>}
              <span style={{ color: theme.warn, fontSize: 13 }}>🔥 {r.calories} kcal</span>
            </div>
          </div>
        ))
      }
  ...
  // HISTORY DETAIL
  const HistDetailScreen = () => {
    const r = activeHistItem;
    if (!r) return null;
    return (
      <div style={{ padding: "0 16px", animation: "fadeIn .3s ease" }}>
        <button style={{ ...s.btn(theme.accentAlt, true), marginBottom: 14, fontSize: 14 }} onClick={() => setScreen("history")}>← Back</button>
        <div style={s.card}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
            <div style={{ fontSize: 36 }}>{r.activity === "Running" ? "🏃" : "🚴"}</div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700 }}>{r.activity}</div>
              <div style={{ color: theme.muted }}>{r.date}</div>
            </div>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {[["⏱ TIME", formatTime(r.time)], ["📏 DIST", formatDist(r.distance)], ["⚡ SPEED", `${(r.speed*3.6).toFixed(1)} km/h`],
              ["🔥 KCAL", r.calories], ...(r.pace ? [["👟 PACE", formatPace(r.pace)]] : [])].map(([l, v]) => (
              <div key={l} style={{ background: theme.surface, borderRadius: 10, padding: "10px 16px", flex: "1 1 120px", textAlign: "center" }}>
                <div style={{ color: theme.muted, fontSize: 11, letterSpacing: 1 }}>{l}</div>
                <div style={{ fontWeight: 700, fontSize: 20, color: theme.accent, fontFamily: "'Space Mono', monospace" }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
        {r.coords && r.coords.length > 2 && (
          <div style={{ ...s.card, height: 220, padding: 8 }}>
            <RouteCanvas coords={r.coords} accent={theme.accent} width={400} height={200} />
          </div>
        )}
      </div>
    );
  ...
  const SettingsScreen = () => (
    <div style={{ padding: "0 16px", animation: "fadeIn .3s ease" }}>
      <div style={s.card}>
        <div style={{ fontSize: 13, color: theme.muted, letterSpacing: 1, marginBottom: 16 }}>PROFILE</div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: theme.muted, marginBottom: 6 }}>NAME</div>
          <div style={{ ...s.input, display: "flex", alignItems: "center", background: theme.surface }}>{user.name}</div>
        </div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: theme.muted, marginBottom: 6 }}>WEIGHT (kg)</div>
          <input style={s.input} type="number" value={settingsWeight} onChange={e => setSettingsWeight(e.target.value)} />
        </div>
      </div>
      <div style={s.card}>
        <div style={{ fontSize: 13, color: theme.muted, letterSpacing: 1, marginBottom: 16 }}>DAILY GOALS</div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: theme.muted, marginBottom: 6 }}>🏃 Running (km)</div>
          <input style={s.input} type="number" value={settingsGoalRun} onChange={e => setSettingsGoalRun(e.target.value)} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 13, color: theme.muted, marginBottom: 6 }}>🚴 Cycling (km)</div>
          <input style={s.input} type="number" value={settingsGoalCyc} onChange={e => setSettingsGoalCyc(e.target.value)} />
        </div>
      </div>
      <div style={s.card}>
        <div style={{ fontSize: 13, color: theme.muted, letterSpacing: 1, marginBottom: 16 }}>APPEARANCE</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontWeight: 600 }}>{darkMode ? "🌙 Dark Mode" : "☀️ Light Mode"}</span>
          <div onClick={() => { setDarkMode(!darkMode); save("ft_dark", !darkMode); }}
            style={{ width: 50, height: 28, borderRadius: 14, background: darkMode ? theme.accent : theme.surface, position: "relative", cursor: "pointer", transition: "background .2s", border: `1px solid ${theme.border}` }}>
            <div style={{ position: "absolute", top: 3, left: darkMode ? 24 : 3, width: 20, height: 20, borderRadius: "50%", background: "#fff", transition: "left .2s" }} />
          </div>
        </div>
      </div>
      <button style={{ ...s.btn(theme.accent), width: "100%", fontSize: 16, marginBottom: 10 }} onClick={saveSettings}>SAVE SETTINGS ✅</button>
      <button style={{ ...s.btn(theme.danger, true), width: "100%", fontSize: 14 }} onClick={() => {
        if (window.confirm("Sab data delete karna hai? 🥺")) {
          localStorage.clear(); setHistory([]); notify("Data cleared!", "warn"); setScreen("login");
        }
  ...
  const activeTab = ["histDetail"].includes(screen) ? "history" : screen;
  ...
  return (
    <>
      ...
      <div style={s.wrap}>
        {/* Header */}
        <div style={s.header}>
          <div style={{ fontSize: 22, fontWeight: 700, letterSpacing: 2, color: theme.accent }}>⚡ FITTRACK</div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {trackState === "running" && <div style={{ color: theme.danger, fontSize: 12, fontWeight: 700, animation: "pulse 1.5s infinite" }}>● TRACKING</div>}
          </div>
        </div>

        {/* Page title */}
        <div style={{ padding: "4px 20px 16px", fontSize: 26, fontWeight: 700, letterSpacing: 1 }}>
          {screen === "home" && `Welcome back! 👋`}
          {screen === "track" && `${activity === "Running" ? "🏃" : "🚴"} ${activity}`}
          {screen === "history" && "Activity Log 📜"}
          {screen === "histDetail" && "Activity Detail 📊"}
          {screen === "settings" && "Settings ⚙️"}
        </div>

        {/* Content */}
        {screen === "home" && <HomeScreen />}
        {screen === "track" && <TrackerScreen />}
        {screen === "history" && <HistoryScreen />}
        {screen === "histDetail" && <HistDetailScreen />}
        {screen === "settings" && <SettingsScreen />}

        {/* Bottom Nav */}
        <div style={s.nav}>
          {tabs.map(t => (
            <button key={t.id} style={s.navBtn(activeTab === t.id)} onClick={() => setScreen(t.id)}>
        ...
        {/* Notification toast */}
        {notification && (
          <div style={{ position: "fixed", top: 16, left: "50%", transform: "translateX(-50%)", background: notification.type === "success" ? theme.accent : notification.type === "warn" ? theme.warn : notification.type === "error" ? theme.danger : theme.accentAlt, color: "#000", padding: "10px 20px", borderRadius: 12, fontWeight: 700, fontSize: 14, zIndex: 999, animation: "slideDown .3s ease", whiteSpace: "nowrap", boxShadow: "0 4px 20px #0008" }}>
            {notification.msg}
          </div>
        )}
      </div>
